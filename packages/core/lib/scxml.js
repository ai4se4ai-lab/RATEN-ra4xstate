...
var evaluateExecutableContent = function (context, _ev, meta, body) {
  var datamodel = context
    ? Object.keys(context)
        .map(function (key) {
          return "const ".concat(key, " = context['").concat(key, "'];");
        })
        .join("\n")
    : "";
  ...
  var fn = new (Function.bind.apply(
    Function,
    __spreadArray(__spreadArray([void 0], __read(args), false), [fnBody], false)
  ))();
  ...
  switch (element.name) {
    ...
    case "assign": {
      return actions.assign(function (context, e, meta) {
        var fnBody = "\n            return {'"
          .concat(element.attributes.location, "': ")
          .concat(element.attributes.expr, "};\n          ");
    ...
    case "send": {
      ...
      var params_1 =
        ...
        element.elements.reduce(function (acc, child) {
          ...
          return ""
            .concat(acc)
            .concat(child.attributes.name, ":")
            .concat(child.attributes.expr, ",\n");
        }, "");
      if (event_1 && !params_1) {
        ...
        convertedEvent = function (context, _ev, meta) {
          var fnBody = "\n              return { type: "
            .concat(event_1 ? '"'.concat(event_1, '"') : eventexpr_1, ", ")
            .concat(params_1 ? params_1 : "", " }\n            ");
      ...
      if ("delay" in element.attributes) {
        ...
        convertedDelay = function (context, _ev, meta) {
          var fnBody = "\n              return ("
            .concat(delayToMs, ")(")
            .concat(element.attributes.delayexpr, ");\n            ");
          return evaluateExecutableContent(context, _ev, meta, fnBody);
        };
      }
...
function toConfig(nodeJson, id, options) {
  ...
  if (nodeJson.elements) {
    ...
    var states = indexedRecord(stateElements, function (item) {
      return "".concat(item.attributes.id);
    });
