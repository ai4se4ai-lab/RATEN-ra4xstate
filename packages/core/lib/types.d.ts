...
type ExtractExtraParameters<A, T> = A extends {
  type: T;
}
  ? ExcludeType<A>
  : never;
type ExtractWithSimpleSupport<
  T extends {
    type: string;
  }
> = T extends any
...
export type InvokeCreator<
  ...
  TOutputEvent extends EventObject = TSourceEvent
> = (
  context: TContext,
  event: TSourceEvent,
  meta: InvokeMeta
...
> = {
  [K in keyof TEventsCausingServices]?:
    ...
    | InvokeCreator<
        ...
        EventObject,
        Cast<TIndexedEvents[keyof TIndexedEvents], EventObject>
      >;
};
