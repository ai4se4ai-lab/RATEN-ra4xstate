...
var Interpreter = /** @class */ (function () {
  ...
  function Interpreter(machine, options) {
    ...
    this.send = function (event, payload) {
      if ((0, utils_1.isArray)(event)) {
        _this.batch(event);
        return _this.state;
      }
      var _event = (0, utils_1.toSCXMLEvent)(
        (0, utils_1.toEventObject)(event, payload)
      );
    ...
    this.sendTo = function (event, to, immediate) {
      ...
      if ("machine" in target) {
        ...
        if (
          _this.status !== InterpreterStatus.Stopped ||
          _this.parent !== target ||
          ...
          var scxmlEvent = __assign(__assign({}, event), {
            name:
              event.name === actionTypes.error
                ? "".concat((0, actions_1.error)(_this.id))
                : event.name,
    ...
    this._exec = function (action, context, _event, actionFunctionMap) {
      ...
      var actionOrExec =
        action.exec ||
        (0, actions_1.getActionFunction)(action.type, actionFunctionMap);
      var exec = (0, utils_1.isFunction)(actionOrExec)
        ? actionOrExec
        : actionOrExec
        ? actionOrExec.exec
        : action.exec;
      if (exec) {
        try {
          return exec(
            context,
            _event.data,
            !_this.machine.config.predictableActionArguments
              ? {
                  action: action,
                  state: _this.state,
                  _event: _event,
      ...
      switch (action.type) {
        ...
        case actionTypes.start: {
          ...
          if (
            ...
            !_this.machine.config.predictableActionArguments &&
            !_this.state.activities[activity.id || activity.type]
          ) {
            break;
          }
          // Invoked services
          if (activity.type === types_1.ActionTypes.Invoke) {
            var invokeSource = (0, utils_1.toInvokeSource)(activity.src);
            var serviceCreator = _this.machine.options.services
              ? _this.machine.options.services[invokeSource.type]
              : undefined;
            var id = activity.id,
              data = activity.data;
            if (!environment_1.IS_PRODUCTION) {
            ...
            var resolvedData = data
              ? (0, utils_1.mapContext)(data, context, _event)
              : undefined;
            if (typeof serviceCreator === "string") {
              // TODO: warn
              return;
            }
            var source = (0, utils_1.isFunction)(serviceCreator)
    ...
    if (this.state.done) {
      ...
      try {
        for (
          ...
          listener((0, actions_1.doneInvoke)(this.id, doneData));
        }
      } catch (e_5_1) {
        e_5 = { error: e_5_1 };
      } finally {
  ...
  Interpreter.prototype.subscribe = function (
    ...
    _, // TODO: error listener
    completeListener
  ) {
    var _this = this;
    var observer = (0, utils_1.toObserver)(
      nextListenerOrObserver,
      _,
      completeListener
    );
    ...
    var resolvedState =
      initialState === undefined
        ...
        : serviceScope.provide(this, function () {
            return (0, State_1.isStateConfig)(initialState)
  ...
  Interpreter.prototype._stopChildren = function () {
    ...
    this.children.forEach(function (child) {
      if ((0, utils_1.isFunction)(child.stop)) {
        child.stop();
      }
    });
    this.children.clear();
  ...
  Interpreter.prototype.stop = function () {
    ...
    scheduler.schedule(function () {
      ...
      var _event = (0, utils_1.toSCXMLEvent)({ type: "xstate.stop" });
      var nextState = serviceScope.provide(_this, function () {
        var exitActions = (0, utils_1.flatten)(
          __spreadArray([], __read(_this.state.configuration), false)
            .sort(function (a, b) {
              return b.order - a.order;
            })
            .map(function (stateNode) {
              return (0,
              actions_1.toActionObjects)(stateNode.onExit, _this.machine.options.actions);
            })
        );
        var _a = __read(
            (0, actions_1.resolveActions)(
              ...
              _this.machine.config.predictableActionArguments
                ...
                : undefined,
              _this.machine.config.predictableActionArguments ||
                _this.machine.config.preserveActionOrder
            ),
            2
          ),
      ...
      _this.update(nextState, _event);
      _this._stopChildren();
      registry_1.registry.free(_this.sessionId);
    });
    return this;
  };
  Interpreter.prototype.batch = function (events) {
    ...
    this.scheduler.schedule(function () {
      ...
      var _loop_1 = function (event_1) {
        var _event = (0, utils_1.toSCXMLEvent)(event_1);
        _this.forward(_event);
        nextState = serviceScope.provide(_this, function () {
          return _this.machine.transition(
            nextState,
            _event,
            undefined,
            exec || undefined
            ...
            __read(
              _this.machine.config.predictableActionArguments
                ...
                : nextState.actions.map(function (a) {
                    return (0, State_1.bindActionToState)(a, nextState);
                  })
            ),
            false
          )
      ...
      _this.update(
        nextState,
        (0, utils_1.toSCXMLEvent)(events[events.length - 1])
      );
    });
  };
  /**
  ...
  Interpreter.prototype.spawn = function (entity, name, options) {
    if (this.status !== InterpreterStatus.Running) {
      return (0, Actor_1.createDeferredActor)(entity, name);
    }
    if ((0, utils_1.isPromiseLike)(entity)) {
      return this.spawnPromise(Promise.resolve(entity), name);
    } else if ((0, utils_1.isFunction)(entity)) {
      return this.spawnCallback(entity, name);
    } else if ((0, Actor_1.isSpawnedActor)(entity)) {
      return this.spawnActor(entity, name);
    } else if ((0, utils_1.isObservable)(entity)) {
      return this.spawnObservable(entity, name);
    } else if ((0, utils_1.isMachine)(entity)) {
      return this.spawnMachine(
        entity,
        __assign(__assign({}, options), { id: name })
      );
    } else if ((0, utils_1.isBehavior)(entity)) {
  ...
  Interpreter.prototype.spawnMachine = function (machine, options) {
    ...
    childService
      .onDone(function (doneEvent) {
        ...
        _this.send(
          (0, utils_1.toSCXMLEvent)(doneEvent, { origin: childService.id })
        );
      })
      .start();
    return actor;
  ...
  Interpreter.prototype.spawnBehavior = function (behavior, id) {
    var actorRef = (0, behaviors_1.spawnBehavior)(behavior, {
      id: id,
      parent: this,
    });
    ...
    promise.then(
      function (response) {
        if (!canceled) {
          ...
          _this.send(
            (0, utils_1.toSCXMLEvent)((0, actions_1.doneInvoke)(id, response), {
              origin: id,
            })
          );
        }
      ...
      function (errorData) {
        if (!canceled) {
          _this.removeChild(id);
          var errorEvent = (0, actions_1.error)(id, errorData);
          try {
            // Send "error.platform.id" to this (parent).
            _this.send((0, utils_1.toSCXMLEvent)(errorEvent, { origin: id }));
          } catch (error) {
            (0, utils_1.reportUnhandledExceptionOnInvocation)(
              errorData,
              error,
              id
            );
    ...
    var actor =
      ((_a = {
        ...
        subscribe: function (next, handleError, complete) {
          var observer = (0, utils_1.toObserver)(next, handleError, complete);
      ...
      (_a[utils_1.symbolObservable] = function () {
        return this;
      }),
      _a);
  ...
  Interpreter.prototype.spawnCallback = function (callback, id) {
    ...
    var receive = function (e) {
      ...
      _this.send((0, utils_1.toSCXMLEvent)(e, { origin: id }));
    };
    var callbackStop;
    try {
      ...
      this.send((0, actions_1.error)(id, err));
    }
    if ((0, utils_1.isPromiseLike)(callbackStop)) {
    ...
    var actor =
      ((_a = {
        ...
        subscribe: function (next) {
          var observer = (0, utils_1.toObserver)(next);
        ...
        stop: function () {
          canceled = true;
          if ((0, utils_1.isFunction)(callbackStop)) {
            callbackStop();
          }
        },
        toJSON: function () {
  ...
  Interpreter.prototype.spawnObservable = function (source, id) {
    ...
    var subscription = source.subscribe(
      function (value) {
        emitted = value;
        _this.send((0, utils_1.toSCXMLEvent)(value, { origin: id }));
      },
      function (err) {
        _this.removeChild(id);
        _this.send(
          (0, utils_1.toSCXMLEvent)((0, actions_1.error)(id, err), {
            origin: id,
          })
        );
      },
      function () {
        _this.removeChild(id);
        _this.send(
          (0, utils_1.toSCXMLEvent)((0, actions_1.doneInvoke)(id), {
            origin: id,
          })
        );
      }
  ...
  Interpreter.prototype.spawnActivity = function (activity) {
    ...
    if (!implementation) {
      if (!environment_1.IS_PRODUCTION) {
        (0, utils_1.warn)(
          false,
          "No implementation found for activity '".concat(activity.type, "'")
        );
      }
      // tslint:disable-next-line:no-console
      return;
  ...
  Interpreter.prototype.attachDev = function () {
    var global = (0, devTools_1.getGlobal)();
    if (this.options.devTools && global) {
      ...
      (0, devTools_1.registerService)(this);
    }
  };
...
var resolveSpawnOptions = function (nameOrOptions) {
  if ((0, utils_1.isString)(nameOrOptions)) {
...
function spawn(entity, nameOrOptions) {
  ...
  return serviceScope.consume(function (service) {
    if (!environment_1.IS_PRODUCTION) {
      var isLazyEntity =
        (0, utils_1.isMachine)(entity) || (0, utils_1.isFunction)(entity);
      (0, utils_1.warn)(
        !!service || isLazyEntity,
        'Attempted to spawn an Actor (ID: "'.concat(
          (0, utils_1.isMachine)(entity) ? entity.id : "undefined",
    ...
    if (service) {
      ...
      return (0, Actor_1.createDeferredActor)(entity, resolvedOptions.name);
    }
  });
}
exports.spawn = spawn;
